...
if (list_1.allT(L3_ast_3.isCExp, es1))
    assert.deepEqual(L3_eval_1.substitute(es1, ["x"], [L3_ast_1.makeNumExp(3)]), ramda_1.map(L3_ast_1.parseL3, ["((lambda (x) (* x x)) 3)"]));
...
if (list_1.allT(L3_ast_3.isCExp, es2) && L3_ast_3.isCExp(vn) && L3_ast_3.isCExp(vf))
    assert.deepEqual(L3_eval_1.substitute([list_1.first(es2)], ["n", "f"], [vn, vf]), [list_1.second(es2)]);
...
if (L3_ast_3.isCExp(lzxz) && L3_ast_3.isCExp(lwzw))
    assert.deepEqual(L3_eval_1.substitute([lzxz], ["x"], [lwzw]), [lzlwzwz]);
// ========================================================
// Tests rename
...
if (L3_ast_3.isCExp(lxx) && L3_ast_3.isCExp(lx1x1))
    assert.deepEqual(L3_eval_1.renameExps([lxx]), [lx1x1]);
...
if (L3_ast_3.isCExp(l1) && L3_ast_3.isCExp(rl1))
    assert.deepEqual(L3_eval_1.renameExps([l1]), [rl1]);
// Test apply proc
